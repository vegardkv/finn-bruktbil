...
- `vegvesen.py` — looks up import status by registration number via Statens Vegvesen's open API. This is the **primary** source for `imported`/`import_country`; the OpenAI description signal is only a fallback when no API key is set or the lookup fails. Requires `SVV_API_KEY`.
